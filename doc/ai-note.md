...
12. 对于一个agent来说，如何设计它的工具系统呢？
      关键问题：
         1. 如何更好的管理工具（可以快速增加工具，也可以识别出失效或低效的工具）?
            工具注册表 + 生命周期状态机
         2.在工具越来越多时，如何保证大模型也能准确的调用？
            工具不进 prompt，只进向量库
         3. 越来越多的工具挤占上下文，导致上下文的腐烂，如何解决？
            分层 + 压缩 + 外置存储
13. RAG完成流程
   `Query Rewrite` -> `Chunk` -> `Embed` -> `Recall` -> `Rerank` -> `Context Pack` -> `Generate` -> `Ground`

logQL
promQL
TraceQL

RAG完整流程： Query Rewrite -> Chunk -> Embed -> Recall -> Rerank -> Context pack -> Generate -> Ground
